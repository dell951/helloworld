...
fetch_all_dup_cmd = "ls -la %s/*-cd2.* | awk '{print $NF}' | gsed -re 's/-cd2//g' > dup_list.txt" % work_path
...
def gen_dup_file(lead_filename, ext):
    run_command("for i in `ls -la %s*%s | awk '{print $NF}'`; do LEN=`expr 'length' : '$i'`; echo $LEN $i; done | sort -n | gsed -re \"s/^.*\s/file '/g\" | gsed -re \"s/$/'/g\"> dup_file.txt " % (lead_filename, ext), True)
